...
- **Opt-out gate**: `sendSms` also checks `contacts.opted_out_at` before sending. The inbound webhook sets this column whenever a customer replies STOP / UNSUBSCRIBE / CANCEL / END / QUIT / STOPALL, and clears it on START / UNSTOP. Sending to an opted-out contact throws `ContactOptedOutError` and logs an `sms.suppressed_opted_out` event. Twilio carriers also block these at their layer, but this gate avoids the 30007 failed-send billing and keeps the AI from drafting replies that will never arrive.
- **Messaging Service routing**: when `TWILIO_MESSAGING_SERVICE_SID` is set, outbound sends pass both `from` and `messagingServiceSid` so Twilio applies the A2P campaign's service-level features (Advanced Opt-Out, sticky sender, smart encoding). The `from` number must be in the service's sender pool — `provisionTenant` handles that on every new number via `attachToMessagingService`.
